...
if (isset($_GET['logout']) && $_GET['logout'] == 'true') {
    ...
    header("Location: entradaSaidaVeiculoSede.php?login=true"); // Redirecionar para a página de login
    exit;
}
...
// Consultar placas da Garagem Sede
if (isset($_GET['action']) && $_GET['action'] == 'get_placas_garagem_sede') {
    try {
        $query = "SELECT placa FROM veiculos WHERE localizacao = 'GARAGEM SEDE'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// Consultar total de vagas e vagas disponíveis por tipo no Estacionamento Sede
if (isset($_GET['action']) && $_GET['action'] == 'get_total_vagas_sede') {
    try {
        $query = "
            SELECT tipo, 
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'GARAGEM SEDE' AND tipo = v.tipo) as total,
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'GARAGEM SEDE' AND tipo = v.tipo AND saida IS NULL) as disponivel
            FROM veiculos v
            WHERE localizacao = 'GARAGEM SEDE'
            GROUP BY tipo";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// Registrar entrada do veículo no Estacionamento Sede
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_entrada_sede') {
    try {
        ...
        $query = "INSERT INTO registro_veiculos (placa, motorista_entrada, horario_entrada, usuario_logado, localizacao, qtd_passageiros) 
                  VALUES (:placa, :motorista_entrada, :horario_entrada, :usuario_logado, 'GARAGEM SEDE', :qtd_passageiros)";
        $stmt = $pdo->prepare($query);
        $stmt->bindParam(':placa', $placa);
...
<body>
    ...
    <script>
        $(document).ready(function () {
            ...
            // Carregar placas da GARAGEM SEDE 
            $.ajax({
                url: 'entradaSaidaVeiculoSede.php',
                method: 'GET',
                data: { action: 'get_placas_garagem_sede' },
                success: function (data) {
                    ...
                    if (veiculos.length > 0) {
                        ...
                        selectPlaca.append('<option value="">Nenhum veículo encontrado no garagem sede</option>');
                    }
            ...
            function atualizarTotalVagas() {
                $.ajax({
                    url: 'entradaSaidaVeiculoSede.php',
                    method: 'GET',
                    data: { action: 'get_total_vagas_sede' },
                    success: function (data) {
            ...
            $('#registrarEntrada').click(function () {
                ...
                $.ajax({
                    url: 'registrarEntradaSede.php', // URL do arquivo PHP para registrar a entrada
                    type: 'POST',
                    data: {
                        placa: placa,
                        modelo: modelo,
                    ...
                    error: function (xhr, status, error) {
                        alert('Erro ao registrar entrada no Estacionamento Sede: ' + error);
                    }
                });
            });
            ...
            $('#registrarOcorrencia').click(function () {
                ...
                var localizacao = 'GARAGEM SEDE'; // Definir a localização fixa para este formulário
                ...
                $.ajax({
                    url: 'registrarOcorrencia.php', // URL do arquivo PHP para registrar a ocorrência
                    method: 'POST',
                    data: {
                    ...
                    error: function (xhr, status, error) {
                        alert('Erro ao registrar ocorrência no Garagem Sede: ' + error);
                    }
                });
            });
        });
        ...
        window.registrarSaida = function(button) {
            ...
            $.ajax({
                url: 'registrarSaidaSede.php', // URL do arquivo PHP para registrar a saída
                type: 'POST',
                data: dados,
                success: function (response) {
        ...
        $("#placa").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: 'entradaSaidaVeiculoSede.php', // Endpoint correto
                    method: 'GET',
                    data: {
