...
if (isset($_GET['logout']) && $_GET['logout'] == 'true') {
    ...
    header("Location: entradaSaidaVeiculoB.php?login=true"); // Redirecionar para a página de login
    exit;
}
...
// Consultar placas da Estacionamento B
if (isset($_GET['action']) && $_GET['action'] == 'get_placas_estacionamento_b') {
    try {
        $query = "SELECT placa FROM veiculos WHERE localizacao = 'ESTACIONAMENTO B'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// Consultar total de vagas e vagas disponíveis por tipo
if (isset($_GET['action']) && $_GET['action'] == 'get_total_vagas_b') {
    try {
        $query = "
            SELECT tipo, 
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'ESTACIONAMENTO B' AND tipo = v.tipo) as total,
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'ESTACIONAMENTO B' AND tipo = v.tipo AND saida IS NULL) as disponivel
            FROM veiculos v
            WHERE localizacao = 'ESTACIONAMENTO B'
            GROUP BY tipo";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// Registrar entrada do veículo no Estacionamento B
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_entrada_b') {
    try {
        ...
        $query = "INSERT INTO registro_veiculos (placa, motorista_entrada, horario_entrada, usuario_logado, localizacao, qtd_passageiros) 
                  VALUES (:placa, :motorista_entrada, :horario_entrada, :usuario_logado, 'ESTACIONAMENTO B', :qtd_passageiros)";
        $stmt = $pdo->prepare($query);
        $stmt->bindParam(':placa', $placa);
...
<body>
    ...
    <li class="nav-item">
            <a class="nav-link active" href="entradaSaidaVeiculoB.php">Registro de Entrada/Saída</a>
        </li>
        
        <li class="nav-item">
            <a class="nav-link active" href="listar_motoristaB.php">Motoristas Oficiais</a>
        </li>
        
        <li class="nav-item">
            <a class="nav-link active" href="listar_acessos_estacionamentoB.php">Acessos Liberados</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="listarEstacionamentoB.php">Veicúlos Liberados</a>
        </li>
       
        <li class="nav-item">
            <a class="nav-link active" href="cadastrarVeiculoEstacionamentoB.php">Cadastrar Veículos</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="cadastrarNovoMotoristaB.php">Novo Cadastro Motorista</a>
        </li>
    </ul>
    <br><br>
    <div class="container">
        <h2>ESTACIONAMENTO B</h2>
    ...
    <script>
        $(document).ready(function () {
            ...
            // Carregar placas da ESTACIONAMENTO B 
            $.ajax({
                url: 'entradaSaidaVeiculoB.php',
                method: 'GET',
                data: { action: 'get_placas_estacionamento_b' },
                success: function (data) {
                    ...
                    if (veiculos.length > 0) {
                        ...
                        selectPlaca.append('<option value="">Nenhum veículo encontrado na estacionamento b</option>');
                    }
            ...
            $('#registrarEntrada').click(function () {
                ...
                $.ajax({
                    url: 'registrarEntradaB.php', // URL do arquivo PHP para registrar a entrada
                    type: 'POST',
                    data: {
                        placa: placa,
                        modelo: modelo,
            ...
            $('#registrarOcorrencia').click(function () {
                ...
                var localizacao = 'ESTACIONAMENTO B'; // Definir a localização fixa para este formulário
                ...
                $.ajax({
                    url: 'registrarOcorrenciaB.php', // URL do arquivo PHP para registrar a ocorrência
                    method: 'POST',
                    data: {
                    ...
                    error: function (xhr, status, error) {
                        alert('Erro ao registrar ocorrência: ' + error);
                    }
                });
            });
        });
        ...
        window.registrarSaida = function(button) {
            ...
            var dados = {
                ...
                horario_saida: formatarDataHora(new Date()) // Formata a data e hora para o padrão do banco de dados
            };
            ...
            $.ajax({
                url: 'registrarSaidaB.php', // URL do arquivo PHP para registrar a saída
                type: 'POST',
                data: dados,
                success: function (response) {
                    ...
                    if (result.success) {
                        ...
                        row.find('input[name="horario_saida[]"]').val(formatarHora(new Date())); // Atualiza o horário de saída no formato HH:MM
        ...
        // Função para formatar a data e hora no padrão do banco de dados
        function formatarDataHora(data) {
            const ano = data.getFullYear();
            const mes = String(data.getMonth() + 1).padStart(2, '0');
            const dia = String(data.getDate()).padStart(2, '0');
        ...
        $("#placa").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: 'entradaSaidaVeiculoB.php', // Endpoint correto
                    method: 'GET',
                    data: {
