...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $localizacao = 'ESTACIONAMENTO B'; // Definir a localização diretamente
    ...
    if ($tipo == 'PARTICULAR' || $tipo == 'MOTO') {
        ...
        $nome = $pdo->query("SELECT nome FROM acessos_liberados WHERE id = $acesso_id")->fetchColumn(); // Buscar o nome do motorista pelo ID
    }

    if (empty($nome) && ($tipo == 'PARTICULAR' || $tipo == 'MOTO')) {
        $_SESSION['erro'] = "O nome do motorista é obrigatório para veículos do tipo PARTICULAR ou MOTO.";
        header("Location: cadastrarVeiculoEstacionamentoB.php");
        exit();
    }
...
$veiculos = $pdo->query("SELECT * FROM veiculos WHERE localizacao = 'ESTACIONAMENTO B'")->fetchAll();

// Buscar os acessos liberados para exibir no formulário
$acessos_query = $pdo->query("SELECT id, nome FROM acessos_liberados WHERE localizacao = 'ESTACIONAMENTO B'");
...
<div class="d-flex justify-content-between align-items-center px-4 py-2 bg-white">
    ...
    <li class="nav-item">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="cadastrarNovoMotoristaB.php">Novo Cadastro</a>
        </li>
    </ul>
    <br><br>
...
<div class="container">
    <h4 class="text-center mt-5">Cadastrar Veículo - ESTACIONAMENTO B</h4>
    ...
    <!-- Formulário de cadastro -->
    <form action="cadastrarVeiculoEstacionamentoB.php" method="POST">
        <div class="row">
            <div class="col-md-3">
