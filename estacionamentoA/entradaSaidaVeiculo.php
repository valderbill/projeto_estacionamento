...
// Consultar placas da Estacionamento A
if (isset($_GET['action']) && $_GET['action'] == 'get_placas_estacionamento_a') {
    try {
        $query = "SELECT placa FROM veiculos WHERE localizacao = 'ESTACIONAMENTO A'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_GET['action']) && $_GET['action'] == 'get_total_vagas_b') {
    try {
        $query = "
            SELECT tipo, 
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'ESTACIONAMENTO A' AND tipo = v.tipo) as total,
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'ESTACIONAMENTO A' AND tipo = v.tipo AND saida IS NULL) as disponivel
            FROM veiculos v
            WHERE localizacao = 'ESTACIONAMENTO A'
            GROUP BY tipo";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// Registrar entrada do veículo no Estacionamento A
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_entrada_b') {
    try {
        ...
        $query = "INSERT INTO registro_veiculos (placa, motorista_entrada, horario_entrada, usuario_logado, localizacao, qtd_passageiros) 
                  VALUES (:placa, :motorista_entrada, :horario_entrada, :usuario_logado, 'ESTACIONAMENTO A', :qtd_passageiros)";
        $stmt = $pdo->prepare($query);
        $stmt->bindParam(':placa', $placa);
...
<body>
    ...
    <li class="nav-item">
            <a class="nav-link active" href="entradaSaidaVeiculo.php">Registro de Entrada/Saída</a>
        </li>
        
        <li class="nav-item">
            <a class="nav-link active" href="listar_motoristaA.php">Motoristas Oficiais</a>
        </li>
        
        <li class="nav-item">
            <a class="nav-link active" href="listar_acessos_estacionamentoA.php">Acessos Liberados</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="listarEstacionamentoA.php">Veicúlos Liberados</a>
        </li>
       
        <li class="nav-item">
            <a class="nav-link active" href="cadastrarVeiculoEstacionamentoA.php">Cadastrar Veículos</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="cadastrarNovoMotoristaA.php">Novo Cadastro Motorista</a>
        </li>
    </ul>
    <br><br>

    <div class="container">
        <h2>ESTACIONAMENTO A</h2>
    ...
    <script>
        $(document).ready(function () {
            ...
            // Carregar placas da ESTACIONAMENTO A 
            $.ajax({
                url: 'entradaSaidaVeiculo.php',
                method: 'GET',
                data: { action: 'get_placas_estacionamento_a' },
                success: function (data) {
                    ...
                    if (veiculos.length > 0) {
                        ...
                        selectPlaca.append('<option value="">Nenhum veículo encontrado na estacionamento a</option>');
                    }
            ...
            $('#registrarEntrada').click(function () {
                ...
                $.ajax({
                    url: 'registrarEntrada.php', // URL do arquivo PHP para registrar a entrada
                    type: 'POST',
                    data: {
                        placa: placa,
                        modelo: modelo,
            ...
            $('#registrarOcorrencia').click(function () {
                ...
                var localizacao = 'ESTACIONAMENTO A'; // Definir a localização fixa para este formulário
                ...
                $.ajax({
                    url: 'registrarOcorrenciaA.php', // URL do arquivo PHP para registrar a ocorrência
                    method: 'POST',
                    data: {
        ...
        window.registrarSaida = function(button) {
            ...
            $.ajax({
                url: 'registrarSaida.php', // URL do arquivo PHP para registrar a saída
                type: 'POST',
                data: dados,
                success: function (response) {
        ...
        $("#placa").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: 'entradaSaidaVeiculo.php', // Endpoint correto
                    method: 'GET',
                    data: {
