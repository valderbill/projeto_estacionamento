...
if (isset($_GET['logout']) && $_GET['logout'] == 'true') {
    ...
    header("Location: entradaSaidaVeiculoSede3.PHP?login=true"); // Redirecionar para a página de login
    exit;
}
...
// Consultar placas da Garagem Sede III
if (isset($_GET['action']) && $_GET['action'] == 'get_placas_garagem_sede_III') {
    try {
        $query = "SELECT placa FROM veiculos WHERE localizacao = 'GARAGEM SEDE III'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// Consultar total de vagas e vagas disponíveis por tipo no Estacionamento Sede III
if (isset($_GET['action']) && $_GET['action'] == 'get_total_vagas_sede_III') {
    try {
        $query = "
            SELECT tipo, 
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'GARAGEM SEDE III' AND tipo = v.tipo) as total,
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'GARAGEM SEDE III' AND tipo = v.tipo AND saida IS NULL) as disponivel
            FROM veiculos v
            WHERE localizacao = 'GARAGEM SEDE III'
            GROUP BY tipo";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
// Registrar entrada do veículo no Estacionamento Sede III
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_entrada_sede_III') {
    try {
        ...
        $query = "INSERT INTO registro_veiculos (placa, motorista_entrada, horario_entrada, usuario_logado, localizacao, qtd_passageiros) 
                  VALUES (:placa, :motorista_entrada, :horario_entrada, :usuario_logado, 'GARAGEM SEDE III', :qtd_passageiros)";
        $stmt = $pdo->prepare($query);
        $stmt->bindParam(':placa', $placa);
...
<body>
    ...
    <li class="nav-item">
            <a class="nav-link active" href="entradaSaidaVeiculoSede3.php">Registro de Entrada/Saída</a>
        </li>
        
        <li class="nav-item">
            <a class="nav-link active" href="listar_motoristaSede3.php">Motoristas Oficiais</a>
        </li>
    
        <li class="nav-item">
            <a class="nav-link active" href="listar_acessos_garagemSede3.php">Acessos Liberados</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="listarSede3.php">Veicúlos Liberados</a>
        </li>
       
        <li class="nav-item">
            <a class="nav-link active" href="cadastrarVeiculoGaragemSede3.php">Cadastrar Veículos</a>
        </li>
        <li class="nav-item">
            <a class="nav-link active" href="cadastrarNovoMotoristaSede3.php">Novo Cadastro</a>
        </li>
    </ul>
    <br><br>
                    ...
                    <div class="col-md-12 mb-3">
                        <div class="card shadow mx-auto" style="max-width: 50rem;">
                            <div class="card-body">
                                ...
                                <p class="card-text fw-bold text-success">🚗 Particulares: <span id="total-particular">45</span> | Disponível: <span id="disponivel-particular">45</span></p>
                                <p class="card-text fw-bold text-primary">🏍️ Motos: <span id="total-moto">20</span> | Disponível: <span id="disponivel-moto">20</span></p>
    ...
    <script>
        $(document).ready(function () {
            // Remover a limpeza do localStorage ao fazer login
            if (window.location.search.includes('login=true')) {
                console.log('Login detectado. Dados no localStorage serão mantidos.');
            }

            // Carregar placas da GARAGEM SEDE III
            $.ajax({
                url: 'entradaSaidaVeiculoSede3.PHP',
                method: 'GET',
                data: { action: 'get_placas_garagem_sede_III' },
                success: function (data) {
                    ...
                    if (veiculos.length > 0) {
                        ...
                        selectPlaca.append('<option value="">Nenhum veículo encontrado no garagem sede III</option>');
                    }
            ...
            function atualizarTotalVagas() {
                $.ajax({
                    url: 'entradaSaidaVeiculoSede3.PHP',
                    method: 'GET',
                    data: { action: 'get_total_vagas_sede_III' },
                    success: function (data) {
                        var vagas = JSON.parse(data);
                        // Atualizar os valores no card
                        vagas.forEach(function (vaga) {
                            if (vaga.tipo === 'PARTICULAR') {
                                $('#total-particular').text(vaga.total);
                                $('#disponivel-particular').text(vaga.disponivel);
                            } else if (vaga.tipo === 'MOTO') {
                                $('#total-moto').text(vaga.total);
                                $('#disponivel-moto').text(vaga.disponivel);
                            } else if (vaga.tipo === 'OFICIAL') {
                                $('#total-oficial').text(vaga.total);
                                $('#disponivel-oficial').text(vaga.disponivel);
                            }
                        });
                    }
                });
            ...
            $('#registrarEntrada').click(function () {
                ...
                $.ajax({
                    url: 'registrarEntradaSede3.PHP', // URL do arquivo PHP para registrar a entrada
                    type: 'POST',
                    data: {
                        placa: placa,
                        modelo: modelo,
                    ...
                    error: function (xhr, status, error) {
                        alert('Erro ao registrar entrada no Estacionamento Sede III: ' + error);
                    }
                });
            });
            ...
            function salvarDadosLocalStorage() {
                ...
                // Salvar informações do card no localStorage
                var cardInfo = {
                    totalParticular: $('#total-particular').text(),
                    disponivelParticular: $('#disponivel-particular').text(),
                    totalMoto: $('#total-moto').text(),
                    disponivelMoto: $('#disponivel-moto').text(),
                    totalOficial: $('#total-oficial').text(),
                    disponivelOficial: $('#disponivel-oficial').text()
                };
            ...
            function carregarDadosLocalStorage() {
                ...
                // Carregar informações do card do localStorage
                var cardInfo = JSON.parse(localStorage.getItem('cardInfo')) || {};
                if (cardInfo.totalParticular) {
                    $('#total-particular').text(cardInfo.totalParticular);
                    $('#disponivel-particular').text(cardInfo.disponivelParticular);
                    $('#total-moto').text(cardInfo.totalMoto);
                    $('#disponivel-moto').text(cardInfo.disponivelMoto);
                    $('#total-oficial').text(cardInfo.totalOficial);
                    $('#disponivel-oficial').text(cardInfo.disponivelOficial);
                }
            }
            ...
            $('#registrarOcorrencia').click(function () {
                ...
                var localizacao = 'GARAGEM SEDE III'; // Definir a localização fixa para este formulário
                ...
                $.ajax({
                    url: 'registrarOcorrencia3.PHP', // URL do arquivo PHP para registrar a ocorrência
                    method: 'POST',
                    data: {
                    ...
                    error: function (xhr, status, error) {
                        alert('Erro ao registrar ocorrência no Garagem Sede III: ' + error);
                    }
                });
            });
        });
        ...
        window.registrarSaida = function(button) {
            ...
            var dados = {
                ...
                horario_saida: formatarDataHora(new Date()) // Formata o horário no padrão 'Y-m-d H:i:s'
            };
            ...
            $.ajax({
                url: 'registrarSaidaSede3.PHP', // URL do arquivo PHP para registrar a saída
                type: 'POST',
                data: dados,
                success: function (response) {
        ...
        // Função para formatar a data e hora no padrão 'Y-m-d H:i:s'
        function formatarDataHora(data) {
            const ano = data.getFullYear();
            const mes = String(data.getMonth() + 1).padStart(2, '0');
            const dia = String(data.getDate()).padStart(2, '0');
        ...
        $("#placa").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: 'entradaSaidaVeiculoSede3.PHP', // Endpoint correto
                    method: 'GET',
                    data: {
