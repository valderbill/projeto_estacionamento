...
if (isset($_GET['logout']) && $_GET['logout'] == 'true') {
    ...
    header("Location: entradaSaidaVeiculoAnexo.php?login=true"); // Redirecionar para a página de login
    exit;
}
...
if (isset($_GET['term'])) {
    ...
    try {
        // Consulta para buscar placas que correspondem ao termo digitado e estão na GARAGEM ANEXO
        $query = "SELECT placa FROM veiculos WHERE placa LIKE :term AND localizacao = 'GARAGEM ANEXO' LIMIT 10";
...
// Consultar placas da Garagem Anexo
if (isset($_GET['action']) && $_GET['action'] == 'get_placas_garagem_anexo') {
    try {
        $query = "SELECT placa FROM veiculos WHERE localizacao = 'GARAGEM ANEXO'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_GET['action']) && $_GET['action'] == 'get_total_vagas') {
    try {
        $query = "
            SELECT tipo, 
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'GARAGEM ANEXO' AND tipo = v.tipo) as total,
                   (SELECT COUNT(*) FROM veiculos WHERE localizacao = 'GARAGEM ANEXO' AND tipo = v.tipo AND saida IS NULL) as disponivel
            FROM veiculos v
            WHERE localizacao = 'GARAGEM ANEXO'
            GROUP BY tipo";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_entrada') {
    try {
        ...
        $query = "INSERT INTO registro_veiculos (placa, motorista_entrada, horario_entrada, usuario_logado, localizacao, qtd_passageiros) 
                  VALUES (:placa, :motorista_entrada, :horario_entrada, :usuario_logado, 'GARAGEM ANEXO', :qtd_passageiros)";
        $stmt = $pdo->prepare($query);
        $stmt->bindParam(':placa', $placa);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_ocorrencia') {
    try {
        ...
        $query = "INSERT INTO ocorrencias (placa_motorista, ocorrencia, localizacao) VALUES (:placa_motorista, :ocorrencia, :localizacao)";
        $stmt = $pdo->prepare($query);
        ...
        echo json_encode(["success" => "Ocorrência registrada com sucesso!"]);
    } catch (PDOException $e) {
        echo json_encode(["error" => "Erro ao registrar ocorrência: " . $e->getMessage()]);
    }
    exit;
}
...
<body>
    ...
    <script>
        $(document).ready(function () {
            ...
            // Carregar placas da GARAGEM ANEXO 
            $.ajax({
                url: 'entradaSaidaVeiculoAnexo.php',
                method: 'GET',
                data: { action: 'get_placas_garagem_anexo' },
                success: function (data) {
                    ...
                    if (veiculos.length > 0) {
                        ...
                        selectPlaca.append('<option value="">Nenhum veículo encontrado na garagem anexo</option>');
                    }
            ...
            $('#registrarEntrada').click(function () {
                ...
                $.ajax({
                    url: 'registrarEntradaAnexo.php', // URL do arquivo PHP para registrar a entrada
                    type: 'POST',
                    data: {
                        placa: placa,
                        modelo: modelo,
            ...
            $('#registrarOcorrencia').click(function () {
                ...
                var localizacao = 'GARAGEM ANEXO'; // Definir a localização fixa para este formulário
                ...
                $.ajax({
                    url: 'registrarOcorrenciaAnexo.php', // URL do arquivo PHP para registrar a ocorrência
                    method: 'POST',
                    data: {
        ...
        window.registrarSaida = function(button) {
            ...
            $.ajax({
                url: 'registrarSaidaAnexo.php', // URL do arquivo PHP para registrar a saída
                type: 'POST',
                data: dados,
                success: function (response) {
        ...
        $("#placa").autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: 'entradaSaidaVeiculoAnexo.php', // Endpoint correto
                    method: 'GET',
                    data: {
